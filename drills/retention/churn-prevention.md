...
Using the `posthog-cohorts` fundamental, analyze churned customers to find common pre-churn patterns. Typical signals:
...
Using the `n8n-triggers` fundamental, create a workflow that runs daily. It queries PostHog for users matching churn signal criteria, scores them, and routes at-risk accounts to the appropriate intervention. Store risk scores in Attio using the `attio-deals` fundamental so your team has visibility.
...
- **Medium risk (score 40-70)**: Triggered email from the `loops-transactional` fundamental. Personalize around the specific signal: "We noticed you haven't used [feature] lately — here's what's new." Include a help link and a calendar booking link.
