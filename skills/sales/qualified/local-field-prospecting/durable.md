...
description: >
    Local Field Prospecting — Durable Intelligence. Visit a couple of coworking hubs and have short
...
Run the `dashboard-builder` drill to create a PostHog dashboard for local-field-prospecting with panels: weekly send volume, reply rate trend, meeting conversion rate, pipeline value from this play, cost per meeting. Set up alerts for when any metric drops below the Scalable-level baseline by more than 20%.
