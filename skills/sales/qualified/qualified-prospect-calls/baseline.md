...
description: >
    Founder calls to prospects — Baseline Run. Outbound calls from the founder to a tight list of
...
## Overview
Founder calls to prospects — Baseline Run. Outbound calls from the founder to a tight list of qualified prospects, from a small smoke test through scaled call volume and agent-driven optimization that sustains or improves meeting rate over time.
...
Run the `posthog-gtm-events` drill to set up event tracking for this play. Configure events: `qualified-prospect-calls_email_sent`, `qualified-prospect-calls_email_replied`, `qualified-prospect-calls_meeting_booked`, `qualified-prospect-calls_linkedin_connected`. Connect PostHog to Attio via webhook so deal stage changes are tracked automatically.
