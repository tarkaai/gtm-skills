...
description: >
    MEDDIC Qualification System — Baseline Run. Apply MEDDIC (Metrics, Economic Buyer, Decision
...
Run the `posthog-gtm-events` drill to set up event tracking for this play. Configure events: `meddic-qualification_email_sent`, `meddic-qualification_email_replied`, `meddic-qualification_meeting_booked`, `meddic-qualification_linkedin_connected`. Connect PostHog to Attio via webhook so deal stage changes are tracked automatically.

### 4. Execute and monitor for 2 weeks
Let the sequences run. Monitor daily: check reply rates, positive vs negative sentiment, bounce rates. Adjust messaging mid-flight if reply rates are below 2% after the first 50 sends.

### 5. Evaluate against threshold
Review PostHog funnel data and Attio deal pipeline. Measure against: >=60% of deals with >=80% MEDDIC completeness and >=20% faster deal velocity over 2 weeks. If PASS, proceed to Scalable. If FAIL, diagnose whether the issue is targeting (wrong ICP), messaging (low reply rate), or conversion (replies but no meetings).
