...
description: >
    AI-Powered Meeting Preparation — Baseline Run. Use AI to research accounts and prepare for sales
...
Run the `posthog-gtm-events` drill to set up event tracking for this play. Configure events: `ai-meeting-prep_email_sent`, `ai-meeting-prep_email_replied`, `ai-meeting-prep_meeting_booked`, `ai-meeting-prep_linkedin_connected`. Connect PostHog to Attio via webhook so deal stage changes are tracked automatically.

### 4. Execute and monitor for 2 weeks
Let the sequences run. Monitor daily: check reply rates, positive vs negative sentiment, bounce rates. Adjust messaging mid-flight if reply rates are below 2% after the first 50 sends.

### 5. Evaluate against threshold
Review PostHog funnel data and Attio deal pipeline. Measure against: >=60% prep time reduction and >=25% higher next-step conversion for AI-prepped calls over 2 weeks. If PASS, proceed to Scalable. If FAIL, diagnose whether the issue is targeting (wrong ICP), messaging (low reply rate), or conversion (replies but no meetings).
