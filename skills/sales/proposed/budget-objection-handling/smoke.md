...
description: >
    Budget Objection Navigation — Smoke Test. Navigate 'no budget' situations by helping prospects
...
Run the `icp-definition` drill to document your Ideal Customer Profile for budget-objection-handling. Define company size, industry, job titles, and pain points. Then run the `build-prospect-list` drill to source 20-50 contacts matching this ICP from Clay. Export the list to Attio CRM.

### 2. Prepare outreach materials
Using the ICP output, draft your budget-objection-handling materials manually. Write 2-3 variants of your core message targeting the specific pain points identified. Keep it scrappy -- this is a Smoke test to validate the channel, not to optimize.
...
Run the `threshold-engine` drill to evaluate results against your pass threshold: Budget objections handled on ≥5 opportunities in 1 week. The threshold engine will pull your logged data from Attio and PostHog, compare against the target, and return PASS or FAIL.
