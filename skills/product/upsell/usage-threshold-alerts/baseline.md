...
description: >
    Plan Limit Notifications — Baseline Run. Alert users approaching plan limits to drive timely
...
Run the `posthog-gtm-events` drill to set up detailed tracking: `usage-threshold-alerts_impression`, `usage-threshold-alerts_engaged`, `usage-threshold-alerts_converted`, `usage-threshold-alerts_retained`. Build PostHog funnels showing the complete user journey through this experience.
...
Measure against: ≥40% upgrade. If PASS, proceed to Scalable. If FAIL, diagnose where users are dropping off and test fixes at that specific point.
