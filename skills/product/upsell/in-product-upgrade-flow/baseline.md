...
description: >
    Self-Serve Upgrade UX — Baseline Run. Self-serve upgrade without sales involvement for
...
Run the `posthog-gtm-events` drill to set up detailed tracking: `in-product-upgrade-flow_impression`, `in-product-upgrade-flow_engaged`, `in-product-upgrade-flow_converted`, `in-product-upgrade-flow_retained`. Build PostHog funnels showing the complete user journey through this experience.
...
Measure against: ≥70% self-serve. If PASS, proceed to Scalable. If FAIL, diagnose where users are dropping off and test fixes at that specific point.
