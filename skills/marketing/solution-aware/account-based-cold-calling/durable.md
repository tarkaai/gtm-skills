...
description: >
    Account-based Cold Calling — Durable Intelligence. Targeted call campaigns to specific
  high-value accounts with personalized research to generate meetings with solution-aware decision
  makers.
...
Run the `dashboard-builder` drill to create a PostHog dashboard for account-based-cold-calling with panels: weekly send volume, reply rate trend, meeting conversion rate, pipeline value from this play, cost per meeting. Set up alerts for when any metric drops below the Scalable-level baseline by more than 20%.
...
Measure against: Sustained call conversion (≥2.5%) over 12 months via AI-driven script optimization and objection handling. This level runs continuously. If metrics sustain or improve, the play is durable. If metrics decay, diagnose whether the issue is market saturation, message fatigue, or ICP drift.
