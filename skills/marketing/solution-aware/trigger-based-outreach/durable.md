...
description: >
    Trigger-based Outreach — Durable Intelligence. Automate outreach based on intent signals like
...
Run the `dashboard-builder` drill to create a PostHog dashboard for trigger-based-outreach with panels: weekly send volume, reply rate trend, meeting conversion rate, pipeline value from this play, cost per meeting. Set up alerts for when any metric drops below the Scalable-level baseline by more than 20%.
...
Measure against: Sustained trigger conversion (≥3%) over 12 months via AI-powered intent detection and timing optimization. This level runs continuously. If metrics sustain or improve, the play is durable. If metrics decay, diagnose whether the issue is market saturation, message fatigue, or ICP drift.
