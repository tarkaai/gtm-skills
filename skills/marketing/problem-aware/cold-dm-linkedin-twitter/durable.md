...
description: >
    Cold DMs on LinkedIn/Twitter — Durable Intelligence. Send a small batch of targeted DMs after
  light engagement to see if DM-only can drive replies and meetings without layering in email.
...
Run the `dashboard-builder` drill to create a PostHog dashboard for cold-dm-linkedin-twitter with panels: weekly send volume, reply rate trend, meeting conversion rate, pipeline value from this play, cost per meeting. Set up alerts for when any metric drops below the Scalable-level baseline by more than 20%.
